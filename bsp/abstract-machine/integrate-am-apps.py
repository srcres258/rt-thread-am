...
struct am_app_thread_args {
  void (*fn)(const char *);
  const char *mainargs;
  Area saved_heap;
};

static void am_app_start_thread(void *raw_args) {
  struct am_app_thread_args *args = (struct am_app_thread_args *) raw_args;
  heap = am_apps_heap;
  args->fn(args->mainargs);
  heap = args->saved_heap;
  rt_free(args);
}
static void am_app_start_wrapper(const char *app_name, void *app_main, int argc, char *argv[]) {
  ...
  struct am_app_thread_args *args = rt_malloc(sizeof(struct am_app_thread_args));
  args->fn = app_main;
  args->mainargs = (argc >= 2 ? argv[1] : "");
  args->saved_heap = heap;
  rt_thread_t tid = rt_thread_create(app_name, am_app_start_thread, args, 0x4000, 15, 20);
